...
def eval_metric(args, metric, count_loss, loss):
    loss /= count_loss
    ...
    F1_Withback = 2 * ((Precision * Recall) / (Precision + Recall))
    MF = np.nanmean(F1_Withback)
    ...
    F_set = {}
    F1_Withback = np.around(F1_Withback, decimals=4)
    for index, per in enumerate(F1_Withback):
        F_set[index] = per

    if args.dataset == 'isprs':
        MIoU = np.sum(PerCiou[:5]) / len(PerCiou[:5])
        P = Precision[:5]
        P_avg = np.sum(P[:5]) / len(P[:5])
        R = Recall[:5]
        R_avg = np.sum(R[:5]) / len(R[:5])
        R = Recall[:5]
        F1_Noback = F1_Withback[:5]
        F1_avg = np.sum(F1_Noback) / len(F1_Noback)
    elif args.dataset == 'paris':
        MIoU = np.sum(PerCiou[1:]) / len(PerCiou[1:])
        P = Precision[1:]
        P_avg = np.sum(P[1:]) / len(P[1:])
        R = Recall[1:]
        R_avg = np.sum(R[1:]) / len(R[1:])
        R = Recall[1:]
        F1_Noback = F1_Withback[1:]
        F1_avg = np.sum(F1_Noback) / len(F1_Noback)
    elif args.dataset == 'cityscapes':
        MIoU = Miou
        P = Precision[:]
        P_avg = np.sum(P[:]) / len(P[:])
        R = Recall[:]
        R_avg = np.sum(R[:]) / len(R[:])
        R = Recall[:]
        F1_Noback = F1_Withback[:]
        F1_avg = np.sum(F1_Withback[:]) / len(F1_Withback[:])
    ...
    with open(result, 'w') as f:
        f.write('MIoU {}   MIoU_Noback {}'.format(str(Miou), MIoU))
        f.write('\nIoU {}'.format(PerCiou_set))
        f.write('\nOA {}'.format(Pa))
        f.write('\nPrecision {} P {} {}'.format(Precision, P, P_avg))
        f.write('\nRecall {} R {} {}'.format(Recall, R, R_avg))
        f.write('\nF1 {} F1 {} {}'.format(F1_Withback, F1_Noback, F1_avg))

    return loss, FWIoU, Miou, MIoU, PerCiou_set, Pa, PerCpa_set, Mpa, MF, F_set, F1_avg
...
def predict_multiscale_sliding(args, model, testLoader, scales, overlap, criterion, mode='predict', save_result=True):
    loss = 0
    count_loss = 0
    model.eval()
    ...
    if mode == 'validation':
        ...
        loss, FWIoU, Miou, MIoU, PerCiou_set, Pa, PerCpa_set, Mpa, MF, F_set, F1_avg = eval_metric(args, metric,
                                                                                                   count_loss, loss)
    else:
        ...
        loss, FWIoU, Miou, MIoU, PerCiou_set, Pa, PerCpa_set, Mpa, MF, F_set, F1_avg = 0, 0, 0, 0, {}, 0, {}, 0, 0, {}, 0
