...
def add_flops_counting_methods(net_main_module):
    """Adds flops counting functions to an existing TNet. After that
    the flops count should be activated and the TNet should be run on an input
    image.
...
def compute_flops(model, input=None):
    ...
    flops = model.compute_average_flops_cost()  # + (TNet.classifier.in_features * TNet.classifier.out_features)
    flops = flops / 1e6 / 2
    return flops
