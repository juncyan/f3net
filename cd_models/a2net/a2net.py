...
class Decoder(nn.Module):
    def __init__(self, mid_d=320, num_classes=2):
...
class LightweightRSCDNet(nn.Module):
    def __init__(self, input_nc=3, num_classes=2):
        super(LightweightRSCDNet, self).__init__()
        self.backbone = mobilenet_v2(in_channels=input_nc,pretrained=False)
    ...
    @staticmethod
    def loss(pred, target_var):
        output, output2, output3, output4 = pred
...
def BCEDiceLoss(inputs, targets):
        ...
        if (targets.shape[1] == 1) and (len(targets==4)) and (len(inputs.shape)==4) and (inputs.shape[1] >1):
            inputs = torch.argmax(inputs, 1, True) 
        inputs = inputs.float()
