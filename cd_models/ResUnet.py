...
class ResUnet(nn.Module):
    def __init__(self, channel=3, classier=2, filters=[32,64, 128, 256, 512]):
        ...
        self.output_layer = nn.Sequential(
            ResidualConv(filters[0],classier,1,1),
            # nn.LogSoftmax(dim=1),
            nn.Sigmoid()
        )
...
class ResUnet_advance(nn.Module):
    def __init__(self, channel=3, filters=[32, 64, 128, 256, 512], cls = 2):
        super(ResUnet_advance, self).__init__()
        ...
        self.output_layer = nn.Sequential(
            ResidualConv(filters[0], cls, 1, 1),
            # nn.LogSoftmax(dim=1),
            nn.Sigmoid()
        )
