...
class DeepLabV3Plus(nn.Module):
    ...
    def __init__(self, in_channel, nclass, backbone='xception', aux=True, pretrained_base=True, dilated=True, **kwargs):
        super(DeepLabV3Plus, self).__init__()
        self.aux = aux
        self.nclass = nclass
        output_stride = 8 if dilated else 32
        self.conv0 = nn.Conv2d(in_channel, 3, 1, 1) #add by tsing 2023/09/26
        self.pretrained = get_xception(pretrained=pretrained_base, output_stride=output_stride, **kwargs)
        # deeplabv3 plus
        self.head = _DeepLabHead(nclass,norm_layer=nn.BatchNorm2d, **kwargs)
        if aux:
...
class _DeepLabHead(nn.Module):
    def __init__(self, nclass, c1_channels=128, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        self.aspp = _ASPP(2048, [12, 24, 36], norm_layer=norm_layer, norm_kwargs=kwargs)
